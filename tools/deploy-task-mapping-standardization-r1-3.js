...
const RELEASE='TASK_MAPPING_STANDARDIZATION_R1_3_PREINSPECT_AUDIT_20260922';
...
const OUTPUT=path.resolve('task-mapping-standardization-r1-3-output');
...
"  return tmStdRefreshPreInspectCalendarAudit_(start, end, 'PREINSPECT_MIRROR_SYNC');",
"}"
].join('\n');

const FN_MANUAL=[
"function refreshStephenPreInspectCandidates() {",
"  return refreshPreInspectCalendarAudit();",
"}"
].join('\n');

const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'tm-std-r1-3-'));
...
try{
  console.log('=== R1.3 1/9 authorize ==='); clasp(['show-authorized-user','--json'],process.cwd());
  console.log('=== R1.3 2/9 PRE clone ==='); clasp(['clone',SCRIPT_ID,'--rootDir','src'],preRoot);
  ...
  if(!preModule.includes('TASK_MAPPING_STANDARDIZATION_R1_2_20260922')) throw new Error('Unexpected PRE standardization version.');
  if(!preModule.includes('tmStdEnsurePreInspectStephenPresence_')) throw new Error('Expected R1.2 Stephen attendee guard missing.');

  console.log('=== R1.3 3/9 build WORK ==='); copy(preRoot,workRoot);
  ...
  const queueFile=replaceFunction(workSrc,'preinspectRefreshStephenCandidateQueue_',FN_QUEUE,changed);
  const manualFile=replaceFunction(workSrc,'refreshStephenPreInspectCandidates',FN_MANUAL,changed);
  ...
  [
    'TASK_MAPPING_STANDARDIZATION_R1_3_PREINSPECT_AUDIT_20260922',
    'PreInspect Calendar Audit',
    'CREATED_BY_OTHER_CLASSIC_FIREPLACE_EMPLOYEE',
    'tmStdRefreshPreInspectCalendarAudit_',
    'Title: Customer # - Customer Name - Phone',
    'CF Preinspects Guest',
    'Stephen Guest',
    'Striven Links'
  ].forEach(marker=>{if(!workModule.includes(marker))throw new Error('Candidate missing marker: '+marker)});

  const queueText=fs.readFileSync(path.join(workSrc,queueFile),'utf8');
  if(!queueText.includes("tmStdRefreshPreInspectCalendarAudit_(start, end, 'PREINSPECT_MIRROR_SYNC')")) throw new Error('Queue audit entrypoint patch missing.');
  const manualText=fs.readFileSync(path.join(workSrc,manualFile),'utf8');
  if(!manualText.includes('return refreshPreInspectCalendarAudit();')) throw new Error('Manual audit entrypoint patch missing.');
  ...
  console.log('=== R1.3 4/9 freshness clone ==='); clasp(['clone',SCRIPT_ID,'--rootDir','src'],freshRoot);
  ...
  console.log('=== R1.3 5/9 push ==='); clasp(['push','--force'],workRoot); pushed=true;
  console.log('=== R1.3 6/9 POST clone ==='); clasp(['clone',SCRIPT_ID,'--rootDir','src'],postRoot);
  ...
  console.log('=== R1.3 7/9 verify hashes/markers ===');
  ...
  if(!postModule.includes('TASK_MAPPING_STANDARDIZATION_R1_3_PREINSPECT_AUDIT_20260922')) throw new Error('POST R1.3 marker missing.');
  if(!postModule.includes('tmStdRefreshPreInspectCalendarAudit_')) throw new Error('POST audit helper missing.');
  const postQueue=fs.readFileSync(path.join(postSrc,queueFile),'utf8');
  if(!postQueue.includes('PREINSPECT_MIRROR_SYNC')) throw new Error('POST scheduled audit hook missing.');

  console.log('=== R1.3 8/9 evidence ===');
  ...
  evidence.runtimeVerification='LIVE_AUDIT_SHEET_VERIFIED_VIA_CALENDAR_AND_SHEETS_CONNECTORS; future refresh hooked into existing PreInspect mirror sync.';
  fs.writeFileSync(evidencePath,JSON.stringify(evidence,null,2));
  console.log('=== R1.3 9/9 complete ==='); console.log('DEPLOYED_SOURCE_VERIFIED');
}catch(err){
