...
if (!manifestPath) throw new Error('Usage: node tools/deploy-selected-row-e2e-r2.js <release-manifest.json>');
...
const targetFileName = String(manifest.targetFile || '94_Selected_Row_End_To_End_R2.js');
...
if (targetFileName !== '94_Selected_Row_End_To_End_R2.js') throw new Error('unexpected targetFile');
...
if (!/DATE_TIME_EXACT_RECONCILE/.test(sourceText) || !/preinspectR3420aReconcileDateTimeFromReview_/.test(sourceText)) {
  throw new Error('candidate source missing exact PreInspection datetime reconciliation route');
}
if (/ScriptApp\.newTrigger\s*\(/.test(sourceText)) throw new Error('candidate may not create triggers');

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'selected-row-e2e-r2-'));
...
const outputDir = path.resolve(repoRoot, 'selected-row-e2e-r2-output');
...
const evidence = {
  schemaVersion: 1,
  release: manifest.release || 'SELECTED_ROW_E2E_R2',
  scriptId,
...
let pushed = false;
try {
  console.log('=== SELECTED ROW E2E R2 1/9 authorize ===');
  clasp(['show-authorized-user', '--json'], repoRoot);

  console.log('=== SELECTED ROW E2E R2 2/9 fresh PRE clone ===');
  ...
  if (!preNames.includes('93_Selected_Row_End_To_End.js')) throw new Error('PRE missing R1 selected-row file');
  ...
  console.log('=== SELECTED ROW E2E R2 4/9 build WORK from PRE ===');
  ...
  console.log('=== SELECTED ROW E2E R2 5/9 freshness clone immediately before push ===');
  ...
  console.log('=== SELECTED ROW E2E R2 6/9 push guarded patch ===');
  clasp(['push', '--force'], workRoot);
  pushed = true;

  console.log('=== SELECTED ROW E2E R2 7/9 POST clone ===');
  ...
  if (JSON.stringify(postNames) !== JSON.stringify(expectedPostNames)) {
    throw new Error('POST file set differs from PRE + expected R2 file');
  }
  ...
  console.log('=== SELECTED ROW E2E R2 8/9 verify allowed change set only ===');
  ...
  console.log('=== SELECTED ROW E2E R2 9/9 complete ===');
